...
function [u1,u2,u1x,u1y,u2x,u2y,sig11,sig12,sig22,f1,f2] = compute_rhs(lambda,mu)
    ...
    u1 = sin(2*pi*y)*(-1 + cos(2*pi*x)) + 1/(mu+lambda)*sin(pi*x)*sin(pi*y);
    u2 = sin(2*pi*x)*(1-cos(2*pi*y)) + 1/(mu+lambda)*sin(pi*x)*sin(pi*y);
    
%     u1 = -sin(pi*x)^3*sin(2*pi*y)*sin(pi*y);
%     u2 = sin(2*pi*x)*sin(pi*x)*sin(pi*y)^3;
