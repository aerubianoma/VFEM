...
if (size(coordinates,2)==3)
    ...
    show_points(middle_points(which,1), middle_points(which,2),middle_points(which,3),which);
else
    %2D case
    middle_points=(coordinates(elements(:,1),:)+coordinates(elements(:,2),:)+coordinates(elements(:,3),:))/3;
    show_points(middle_points(which,1), middle_points(which,2),0*middle_points(which,1),which);
end
